...
使用 Dockerfile 定制镜像，添加 `docker` 命令行可执行文件。
...
我们可以用这样的命令构建镜像，假设镜像名为 `docker-cli`：

```bash
$ docker build -t docker-cli .
```

然后我们在启动容器的时候，将宿主的 `/var/run/docker.sock` 文件挂载到容器的同样位置，从而让容器内可以通过 [unix socket](https://en.wikipedia.org/wiki/Unix_domain_socket) 调用宿主的 Docker 引擎。
...
$ docker run -it --rm \
    ...
    docker-cli \
    bash
root@c2d6cd765976:/# docker version
Client:
 Version:      1.12.3
 API version:  1.24
 Go version:   go1.6.3
